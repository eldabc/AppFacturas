    	...
    	if ($form->isSubmitted()) {
    		...
    		if (!$providerEntity) {
    			...
    			foreach ($entity->getItems() as $item){
					...
					if(is_null($item->getStock())){ 
						// echo "<br>";
						// echo $item->getQuantity();
						// echo "<br>";
						// echo $item->getStock();
						// echo "replace ".str_replace('',77,$item->getQuantity());
						// $item->setStock(intval(str_replace(null,$item->getQuantity(),$item->getStock()) ));
						// $em->persist($entity);
						$var = $item->getQuantity();
						// $item->setStock($var);
