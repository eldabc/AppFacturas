...
function validateExistInStock(path, product, quantity){
	
	const promise = new Promise(function (resolve, reject) {
		$.post( path, { product: product } ).success(function( data ) {
			if(data != ''){
				resolve(data[0].stock_item);
			}
		}).fail(function() {
			reject(new Error('Problemas de stock'));
		});
	})
