    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            foreach($invoice->getItems() as $item){
                ...
                $globalStock = $this->getDoctrine()->getRepository('SiwappProductBundle:Product')->validateStock($item->getProduct());

                // echo "<br>count " .sizeof($globalStock);
                    // dump( $globalStock);

                $ItemStock = $this->getDoctrine()->getRepository('SiwappProductBundle:Product')->findLikeReferenceSale($item->getProduct());
                
                    // dump($ItemStock);
                
                // validacion globalStock
                if( $item->getQuantity() > $globalStock[0]['stock_item']){
                    // dump('mayor');

                    $this->addTranslatedMessage('Solo quedan '.$globalStock[0]['stock_item'].' unidades del producto '.$item->getProduct()->getReference().' en stock', 'warning');
                ...
                $resta = 0;  
                $update = true;     
                foreach($ItemStock as $key => $value ){
                    // echo $resta;
                    if($value->stock > 0) {
                    ...
                    $resta =  ($value->stock - $quantity);

                    // dump( $resta);
                    // dump( $value->stock);
                    // dump( $quantity); //$item->getQuantity());
                    // dump("value->id ". $value->id);  
                    $newItem = new Item;                      
                        if($resta > 0){ 
                            $tomado = $value->stock - $resta;
                                                 
                            if($update){ 
                                dump("valor positivo ". $resta);
                                $conn =  $this->getDoctrine()->getEntityManager()->getConnection(); 
                                $rowsAffected = $conn->executeUpdate("update item set stock = ".$resta." where id = ".$value->id);
                                $update = false;

                                $newItem = new Item;
                                    $newItem->setProduct($item->getProduct());
                                    $newItem->setDescription($item->getDescription());
                                    $newItem->setQuantity($tomado);
                                    $newItem->setDiscount($item->getDiscount());
                                    $newItem->setUnitaryCost($item->getUnitaryCost());
                                    $newItem->setItemIdProvider($value->id);
                            ...
                            dump("valor negativo ". $resta);
                            $resta = $resta * -1;
                            $tomado = $value->stock;

                                $conn =  $this->getDoctrine()->getEntityManager()->getConnection(); 
                                   $conn->executeUpdate("update item set stock = 0 where id = ".$value->id);
            ...
            try {
                $em->persist($invoice);
                $em->flush();           
                    $query = $em->createQuery("DELETE FROM Siwapp\CoreBundle\Entity\Item e WHERE e.expirationDate IS NULL and e.nLote IS NULL and e.stock IS NULL and e.item_id_provider IS NULL "); $query->execute();
    ...
    public function editAction(Request $request, $id)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            foreach ($entity->getItems() as $item){
             ...
             if(($globalStock[0]['stock_item']+$changeset['quantity'][0]) >= abs($item->getQuantity())){
                        ...
                        if($item->getItemIdProvider()){
                                $resta = $changeset['quantity'][0] - $changeset['quantity'][1];
                                $conn =  $this->getDoctrine()->getEntityManager()->getConnection(); 
                                $rowsAffected = $conn->executeUpdate("update item set stock = stock + ".$resta." where id = ".$item->getItemIdProvider());
                        }else{
                            // new item edit
                            // $newItem = new Item;
                            // $newItem->setProduct($item->getProduct());
                        ...
                        $this->addTranslatedMessage('Solo quedan '.($globalStock[0]['stock_item']+$changeset['quantity'][0]).' unidades del producto '.$item->getProduct()->getReference().' en stock', 'warning');
